...
void retro_init(void)
{
    ...
    else
    {
        ...
        if (environ_cb(RETRO_ENVIRONMENT_SET_PIXEL_FORMAT, &fmt))
        {
            S9xMessage(S9X_MSG_INFO, S9X_CATEGORY_EXTERNAL, "RGB565 rejected by frontend; fell back to 0RGB1555.");
        }
    }
...
static inline void s9x_ps2_fix_colors(uint16_t *pixels, int width, int height, int pitch)
{
    ...
    for (int y = 0; y < height; y++)
    {
        ...
        {
            uint16_t p = row[x];
            // 1. Swap 16-bit word endianness (byteswap)
            p = (p >> 8) | (p << 8);
            // 2. Swap Red and Blue channels while preserving the 6-bit Green channel
...
void S9xDeinitUpdate(int width, int height)
{
    ...
    else if (snes_ntsc_filter)
    {
        ...
        s9x_ps2_fix_colors((uint16_t *)ntsc_screen_buffer, ntsc_out_width, height, ntsc_out_pitch);
    ...
    else if (S9xHdPackActive())
    {
        ...
        if (hd_frame)
        {
            s9x_ps2_fix_colors(hd_frame, hd_w, hd_h, hd_pitch);
            video_cb(hd_frame, hd_w, hd_h, hd_pitch);
        }
        else
        {
            void *target_buffer = sw_fb_active ? sw_fb_data : GFX.Screen;
            int target_width = sw_fb_active ? sw_fb_width : width;
            int target_height = sw_fb_active ? sw_fb_height : height;
            int target_pitch = sw_fb_active ? sw_fb_pitch : GFX.Pitch;

            s9x_ps2_fix_colors((uint16_t *)target_buffer, target_width, target_height, target_pitch);
            video_cb(target_buffer, target_width, target_height, target_pitch);
        }
    }
    else
    {
        ...
        if (sw_fb_active)
        {
            s9x_ps2_fix_colors((uint16_t *)sw_fb_data, sw_fb_width, sw_fb_height, sw_fb_pitch);
            video_cb(sw_fb_data, sw_fb_width, sw_fb_height, sw_fb_pitch);
        }
        else
        {
            s9x_ps2_fix_colors((uint16_t *)GFX.Screen, width, height, GFX.Pitch);
